      ...
      <View style={styles.outContainer}>
        <View style={styles.container}>
          ...
          <ScrollView contentContainerStyle={styles.inContainer}>
              ...
              <SelectMultiple
                ...
                onSelectionsChange={selected => {
                  this.onChangeSettings({
                    zmanimToShow: ZmanTypes.filter(zt =>
                      selected.some(i => i.value === zt.name)
                    )
                  });
                }}
                style={styles.selectMultipleStyle}
                rowStyle={styles.selectMultipleRowStyle}
                checkboxStyle={styles.selectMultipleCheckboxStyle}
                selectedCheckboxStyle={
                  styles.selectMultipleSelectedCheckboxStyle
                }
                selectedRowStyle={styles.selectMultipleSelectedRowStyle}
                labelStyle={styles.selectMultipleLabelStyle}
                selectedLabelStyle={styles.selectMultipleSelectedLabelStyle}
              />
            </View>
            ...
            <View style={styles.numBoxView}>
              <Picker
                ...
                onValueChange={numberOfItemsToShow =>
                  this.onChangeSettings({ numberOfItemsToShow })
                }
              >
                {range(1, 10).map(num => (
            ...
            <View style={styles.numBoxView}>
              <Picker
                ...
                onValueChange={minToShowPassedZman =>
                  this.onChangeSettings({ minToShowPassedZman })
                }
              >
                {range(0, 60).map(num => (
                  <Picker.Item key={num} value={num} label={num.toString()} />
            ...
            <Text>
              השעה עכשיו: {Utils.getTimeString(this.props.nowTime, true)}
            </Text>
          ...
          <Text
            ...
            onLongPress={() =>
              AppUtils.changeSystemHomeSettings(this.props.settings)
            }
          >
            סגור X
          </Text>
